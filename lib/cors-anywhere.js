...
var net = require('net');
var url = require('url');
var tracer = require('@opentelemetry/api');
...
function trace(callback, name, attributes) {
  const host = undefined;
  try {
    host = new URL(attributes.location).host
  } catch (error) {

  }
  var tracService = tracer.trace.getTracer('cors-anywhere-service');
  var span = tracService.startSpan(name, {
      ...
      pipe: function (proxyReq) {
        ...
        proxyReq.on = function (eventName, listener) {
          ...
          return proxyReqOn.call(this, 'response', function (proxyRes) {
            if (trace(() => onProxyResponse(proxy, proxyReq, proxyRes, req, res), onProxyResponse.name, { location: location?.href })) {
              try {
                listener(proxyRes);
              } catch (err) {
    ...
    if (parsedLocation) {
      if (statusCode === 301 || statusCode === 302 || statusCode === 303) {
        ...
        if (requestState.redirectCount_ <= requestState.maxRedirects) {
          ...
          trace(() => proxyRequest(req, res, proxy), proxyRequest.name, { location: req?.corsAnywhereRequestState?.location?.href });
          return false;
        }
      }
...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    trace(() => processCall(req, res, corsAnywhere), processCall.name, { location: req?.url?.slice(1) });
  };
}

function processCall(req, res, corsAnywhere) {
